...
useEffect(() => {
    const storedJob = getStoredJobApplication();
    if(jobs.length > 0) {
        // const jobApplied = jobs.filter(job => storedJob.include(job.id) )
        const jobApplied = [];
            ...
            <div>
                {
                    ...
                    ><span><p className="mr-5">{job.job_title}</p>{job.company_name} : {job.remote_or_onsite} </span></li>)
                }
            </div>
        </div>
    );
